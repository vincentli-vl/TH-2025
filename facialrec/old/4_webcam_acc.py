...
while True:
    ...
    if analysis and isinstance(analysis, list):
        ...
        if is_valid_face:
            ...
            #retrieving the confidence of the dominant emotion
            emotion_confidence = latest_emotions.get(dominant_emotion, 0)

            #formatting the percentage text -> acc for accuracy
            emotion_percent_text = f"Acc: {emotion_confidence:.1f}%"
            ...
            #add the percentage of the dominant emotion
            percent_font_scale = 0.8
            percent_thickness = 2
            percent_size, _ = cv2.getTextSize(emotion_percent_text, cv2.FONT_HERSHEY_SIMPLEX, percent_font_scale, percent_thickness)

            #positioning the percentage in bottom right, slightly below box
            percent_X = x+ w - percent_size[0]
            percent_y = y+ h + 25

            cv2.putText(frame, emotion_percent_text, (percent_X, percent_y),
                        cv2.FONT_HERSHEY_SIMPLEX, percent_font_scale, CYAN, percent_thickness)
